...
const passKey = 'a6489554-f07d-404d-8dd6-b6ea9c307b28'
export default {
encrypt (obj) {
    Object.keys(obj).forEach(key => {
      if (key === 'value' || key === 'name' || key === 'description' || key === 'code' || key === 'password') {
        obj[key] = CryptoJS.AES.encrypt(JSON.stringify(obj[key]), passKey).toString()
      }
    })
    return obj
   },
    ...
    Object.keys(obj).forEach(key => {
      if (key === 'value' || key === 'name' || key === 'description' || key === 'code' || key === 'password') {
       ...
       // console.log(CryptoJS.AES.decrypt(JSON.stringify(obj[key]), this.passKey).toString(CryptoJS.enc.Utf8))
        obj[key] = CryptoJS.AES.decrypt(JSON.stringify(obj[key]), passKey).toString(CryptoJS.enc.Utf8)
      }
    })
    return obj
   },
   ...
   // console.log(CryptoJS.AES.encrypt(text, this.passKey).toString())
    text = CryptoJS.AES.encrypt(text, passKey).toString()
  ...
  decryptPlainText (text) {
   ...
   // text = CryptoJS.AES.encrypt(text, this.passKey).toString()
    text = CryptoJS.AES.decrypt(text, passKey).toString(CryptoJS.enc.Utf8).replace('"', '').replace('"', '')
    return text
  }
}
